...
function generateCode() {
    ...
    const possible = 'ABCDEFGHIJKLMNOPQRSTUVXZabcdefghijklmnopqrstuvz0123456789';
...
async function getLink(req : Request, res : Response) {
    ...
    const link =  await linksRepository.findByCode(code);  //links.find(item => item.code === code);
    if (!link) {
        res.sendStatus(404);
    } else {
        res.json(link);
...
async function hitLink(req : Request, res : Response) {
    const code = req.params.code as string;
    const link =  await linksRepository.hit(code); //links.findIndex(item => item.code === code);
